...
void schedule_block(float matrix[12][48], int par_factor, int code, ofstream& outfile){
	...
	//for(int k = 0; k < 4 ; k++){
		...
		for(int i = 0; i <  par_factor; i++){
			...
			for(int j = 0; j < MAX_NON_ZERO; j++){
				if(j != MAX_NON_ZERO - 1){
					outfile << "{" << schedule_matrix[i][j].value << ","<<schedule_matrix[i][j].row<<","<<schedule_matrix[i][j].col % (48 / par_factor)<<"},\n";
				}
				else{
					outfile << "{" << schedule_matrix[i][j].value << ","<<schedule_matrix[i][j].row<<","<<schedule_matrix[i][j].col % (48 / par_factor)<<"}\n";
				}
...
//		}
	//}
	outfile << "};\n";
	outfile << "ap_uint<6> max"+ to_string(code) + "[" + to_string(par_factor) + "] ={";
	for(int i = 0; i < par_factor; i++){
		if(i != par_factor - 1){
			outfile << max[i] <<",";
		}
